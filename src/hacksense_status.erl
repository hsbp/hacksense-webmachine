...
to_eeml(ReqData, {_, #status{timestamp=TimeStamp, status=Status}} = State) ->
    ...
    Env = {environment, [{updated, binary:replace(TimeStamp, <<" ">>, <<$T>>)}],
           [{title, ["Hacksense Budapest"]}, {feed, ["https://vsza.hu/hacksense/eeml_status.xml"]},
            {status, ["live"]}, {description, ["Hacksense Hackerspace Budapest"]},
            {icon, ["https://hsbp.org/icon.png"]}, {website, ["https://hsbp.org/"]},
            Location, Data]},
...
item_to_spaceapi(#status{status=S, timestamp=TS}) ->
    ...
    Projects = ['https://github.com/hsbp',
                'https://hsbp.org/projects', 'https://hsbp.org/hwprojektek'],
	...
	Feeds = #{
		?RSS_FEED(wiki, "https://hsbp.org/tiki-wiki_rss.php?ver=2"),
		?RSS_FEED(calendar, "https://hsbp.org/tiki-calendars_rss.php?ver=2"),
		?RSS_FEED(blog, "https://hsbp.org/tiki-blogs_rss.php?ver=2")},
